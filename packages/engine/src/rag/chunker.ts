  ...
  function scan(dir: string, depth = 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (lowerName.includes('lock') || lowerName.endsWith('.min.js') || lowerName.endsWith('.map') || lowerName.endsWith('.svg')) continue;

        if (allChunks.length >= 1500) return; // Hard safety cap to prevent Heap OOM
