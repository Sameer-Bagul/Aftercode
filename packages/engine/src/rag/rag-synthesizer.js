...
export async function runRagMultiPassSynthesis(evidence, hybridIndex) {
...
async function executeGeminiRagSynthesis(evidence, retrievedChunks, apiKey) {
...
DO NOT use emojis.`;
    ...
    const modelsToTry = ['gemini-3.6-flash', 'gemini-2.5-flash', 'gemini-3.6-pro'];
    let response = null;
    let lastErr = null;
    ...
    if (jsonMatch) {
        ...
        return {
            ...
            contributions: (parsed.contributions || []).map((s) => stripEmojis(s)),
            features: (parsed.features || []).map((s) => stripEmojis(s)),
            challenges: (parsed.challenges || []).map((s) => stripEmojis(s)),
            learnings: (parsed.learnings || []).map((s) => stripEmojis(s)),
            futureRoadmap: (parsed.futureRoadmap || []).map((s) => stripEmojis(s)),
            ragRetrievedChunksCount: retrievedChunks.length,
        };
    }
    throw new Error('Failed to parse JSON from Gemini RAG SDK response');
}
function executeHeuristicRagSynthesis(evidence, retrievedChunks) {
    ...
    if (techStack.frontend.length > 0)
        contributions.push(`Engineered presentation layer with ${techStack.frontend.join(', ')}`);
    if (techStack.backend.length > 0)
        contributions.push(`Built backend router and service controllers with ${techStack.backend.join(', ')}`);
    if (techStack.database.length > 0)
        contributions.push(`Configured data persistence and schema models using ${techStack.database.join(', ')}`);
    if (techStack.aiMl.length > 0)
        contributions.push(`Integrated AI/ML inference runtime powered by ${techStack.aiMl.join(', ')}`);
    if (apiEndpoints.length > 0)
        contributions.push(`Exposed ${apiEndpoints.length} REST/WebSocket API endpoints`);
