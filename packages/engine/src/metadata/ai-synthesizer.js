...
export function generateMermaidArchitectureDiagram(evidence) {
...
export function generateExhaustiveTechnicalDescription(evidence) {
...
export async function runMultiPassSynthesis(evidence) {
...
function runDeepHeuristicSynthesis(evidence) {
...
export function truncateToTokenBudget(text, maxChars = 15000) {
    if (!text || text.length <= maxChars)
        return text;
    return `${text.substring(0, maxChars)}\n\n[Content truncated to maintain AI token safety budget]`;
}
async function runLlmMultiPassSynthesis(evidence, apiKey) {
