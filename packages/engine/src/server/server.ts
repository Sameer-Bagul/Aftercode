  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/repos' && req.method === 'GET') {
      try {
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: true, totalRepositories: repos.length, repos }));
      } catch (err: any) {
    ...
    // POST /api/video/tts/:slug
    if (url.pathname.startsWith('/api/video/tts/') && req.method === 'POST') {
      const slug = url.pathname.replace('/api/video/tts/', '').trim();
      try {
        console.log(` 🎙️ [Server API] Triggering Local TTS Voiceover Synthesis for '${slug}'...`);
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: true, slug, ttsResult }));
      } catch (err: any) {
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: false, error: err.message }));
      }
    ...
    // POST /api/video/render/:slug
    if (url.pathname.startsWith('/api/video/render/') && req.method === 'POST') {
      const slug = url.pathname.replace('/api/video/render/', '').trim();
      try {
        console.log(` 🎬 [Server API] Triggering Remotion Video Render for '${slug}'...`);
        const paths = initVideoWorkspace(workspaceDir, slug);
        const scenes = [
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: true, slug, renderResult }));
      } catch (err: any) {
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: false, error: err.message }));
      }
