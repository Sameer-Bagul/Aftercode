...
export async function prepareWorkspace(workspaceDir) {
...
export async function cloneRepositoryToWorkspace(repoUrl, workspaceDir) {
    ...
    try {
        ...
        console.warn(` ⚠️ [Step 1/7] Git clone encountered error (${err.message}). Switching to GitHub API Tarball Streaming Fallback...`);
        try {
            await prepareWorkspace(workspaceDir);
            ...
            throw new Error(`Failed to extract repository ${repoUrl}: ${fallbackErr.message}`);
        }
    }
}
export async function wipeWorkspace(workspaceDir) {
