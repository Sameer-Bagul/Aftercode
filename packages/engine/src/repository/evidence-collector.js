...
export function collectRepositoryEvidence(workspaceDir, repoName) {
    const techStack = {
        frontend: [],
        backend: [],
        database: [],
            ...
            if (entry.isDirectory()) {
                ...
                if (lower === 'package.json') {
                    ...
                    try {
                        ...
                        if (deps['vue'])
                            techStack.frontend.push('Vue');
                        if (deps['tailwindcss'])
                            techStack.frontend.push('TailwindCSS');
                        if (deps['express'])
                        ...
                        if (deps['openai'])
                            techStack.aiMl.push('OpenAI API');
                        if (deps['@google/genai'] || deps['@google/generative-ai'])
                            techStack.aiMl.push('Gemini AI');
                        if (deps['onnxruntime-node'] || deps['onnxruntime-web'])
                            techStack.aiMl.push('ONNX Runtime');
                        if (deps['vitest'] || deps['jest'])
                            techStack.testing.push('Vitest/Jest');
                        if (deps['typescript'])
                            techStack.tools.push('TypeScript');
                        if (deps['eslint'])
                            techStack.tools.push('ESLint');
                    }
                    catch {
                        // Ignore parse errors
                    }
                ...
                if (lower === 'requirements.txt' || lower === 'pipfile' || lower === 'pyproject.toml') {
                    ...
                    try {
                        ...
                        if (content.includes('torch') || content.includes('pytorch'))
                            techStack.aiMl.push('PyTorch');
                        if (content.includes('onnx'))
                            techStack.aiMl.push('ONNX');
                        if (content.includes('langchain'))
                            techStack.aiMl.push('LangChain');
                        if (content.includes('sqlalchemy'))
                            techStack.database.push('SQLAlchemy');
                    }
                    catch {
                        // Ignore
                    }
    ...
    if (hasFrontend)
        userFlowSteps.push('User accesses application user interface via browser');
    if (apiEndpoints.length > 0)
        userFlowSteps.push('User triggers action invoking HTTP/WebSocket API endpoints');
    if (hasAi)
        userFlowSteps.push('Payload processed through AI/ML pipeline or local inference model');
    if (hasDb)
        userFlowSteps.push('Data persisted or retrieved from database storage layer');
    if (userFlowSteps.length === 0)
        userFlowSteps.push('User executes CLI or entrypoint script');
    codeFlowSteps.push('Client or caller initiates request to application entrypoint');
    if (hasBackend)
        codeFlowSteps.push('Backend router validates request parameters and applies middleware');
    if (hasAi)
        codeFlowSteps.push('Service layer delegates task to ML runtime or LLM API worker');
    if (hasDb)
        codeFlowSteps.push('Database ORM/client executes storage query');
