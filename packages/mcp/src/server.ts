  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
              ...
              text: JSON.stringify(
                {
                  ...
                  inventoryPath,
                  repositories: state.repositories.map((r: any) => ({ name: r.name, slug: r.slug, url: r.url, fork: r.fork })),
                },
                null,
                2
              ),
      ...
      if (name === 'aftercode_create_video_workspace') {
        const slug = String((args as any).slug);
        const template = String((args as any).template || 'technical-saas');
        const projectRoot = process.cwd();
        const workspaceDir = path.join(projectRoot, 'workspace', 'current');
...
// Run direct if called
if (process.argv[1] && process.argv[1].endsWith('server.js')) {
  runAftercodeMcpServer();
}
